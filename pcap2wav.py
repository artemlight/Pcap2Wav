...
class SipMessage:
    ...
    def parse_headers(self):
        ...
        for line in self.data.splitlines(keepends=True):
            ...
            line = line.strip()
            if line == '':
                self.content = self.data[current_offset + line_size:]
                return
            # Method
            if line_number == 0:
                pass
...
class SipSession:
    def __init__(self, callid):
        ...
        self.target_ip = None
        self.packets_with_sdp = list()

    def AddMessage(self, msg):
        self.requests.append(msg)
        if not (msg.sdp_data is None):
            # Add current packet index to packet with sdp list
            self.packets_with_sdp.append(len(self.requests) - 1)
...
# Main routine
def pcap2wav(param_sip_file, param_rtp_file, param_out_file, new_tcpdump_format):
    file_sip = gzip.open(param_sip_file, 'rb')
    pcap = dpkt.pcap.Reader(file_sip)
    sip_sessions = {}
    for ts, buf in pcap:
        if new_tcpdump_format:
            ip = dpkt.ip.IP(buf[16:])
        else:
            eth = dpkt.ethernet.Ethernet(buf)
            ip = eth.data
        # Check if a new TCPDump format
        # eth = dpkt.ethernet.Ethernet(buf)
        ...
        # Skip all non-udp packets
        if ip.p != socket.IPPROTO_UDP:
            continue

        # Only parse port 5060
        ...
        # Group all SIP packets by Call IDs
    ...
    for session in sip_sessions:
        ...
        print('     RTP Streams:')
        if len(sip_sessions[session].packets_with_sdp) != 2:
            print('Error: more or less than 2 RTP streams per channel!')
            # exit()
        for sdp_request_index in sip_sessions[session].packets_with_sdp:
            print('     ',
    ...
    for session_id in sip_sessions:
        ...
        for sdp_index in session.packets_with_sdp:
        ...
        for abs_ts, buf in pcap:
            ...
            if ts < jb_start_ms:
                print("Packet time:", ts, " Jitterbuffer range:", jb_start_ms, jb_start_ms + jb_size_ms * 2,
                      ': Skipped.')
                continue
            ...
            # Check if additional TCPDump header is present
            if new_tcpdump_format:
                ip = dpkt.ip.IP(buf[16:])
            else:
                eth = dpkt.ethernet.Ethernet(buf)
            ...
            # Skip all non-interesting packets
            if ip.p != socket.IPPROTO_UDP:
                continue
            if ip.udp.dport not in rtp_ports:
                continue
